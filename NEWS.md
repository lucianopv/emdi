...
* emdi no longer changes OpenMP settings for the rest of the R session. Thread counts are applied with a `num_threads()` clause scoped to each parallel region rather than by calling `omp_set_num_threads()`, so emdi cannot affect other packages' threading. The unexported `set_omp_threads()`/`get_omp_threads()` helpers are removed; use `cpus` or `options(emdi.cores=)`.
...
* New C++ fast path for the Jiang jackknife MSE in `fh()` (`mse_type = "jackknife"`, `method = "reml"` without a correlation structure). The delete-one-domain loop previously rebuilt a full model framework in R for every domain; it now runs entirely in C++ on the direct estimates, model matrix and sampling variances, reusing the existing REML and EBLUP kernels. On `eusilcA` (94 domains, arcsin transformation) this takes a jackknife fit from a median 7.76s to 0.068s, a 114-fold speedup, with MSE agreeing with the R path to 8.8e-11 relative. The loop contains no random number generation, so results are exactly reproducible regardless of the number of OpenMP threads. `method = "ml"`, the adjusted-REML family, the measurement-error model and the spatial model each estimate the variance component differently and continue to use the R implementation.
* Progress reporting for the long-running MSE estimators (`ebp()` bootstrap, `fh()` jackknife and spatial bootstrap) now shows elapsed time and a projected finish time on a single line that updates in place, instead of one line per iteration reporting only the iteration number. When output is redirected to a log file rather than a terminal it falls back to periodic whole lines. This also fixes an older progress message in the `ebp()` bootstrap that was built across two source lines, embedding a stray tab and line break.
* Internal refactor: the three separate copies of Brent's method that had accumulated in the C++ sources (`fh_reml.cpp`, `reml_optimization.cpp`, `lme_fit.cpp`) are consolidated into a single implementation in `src/brent.h`. The retained implementation is the faithful transcription of R's `Brent_fmin`, which is what allows the C++ kernels to be compared against `stats::optimize()` to optimizer tolerance. `sigmau2` and the EBLUP quantities for `fh()`, and the `lme_fit_cpp()` variance components and BLUPs, are bit-identical before and after; the transformation parameter `lambda` and `ebp()` point estimates move by at most 5.6e-07 and 2.4e-07 relative respectively, from the slightly different iteration path. Also adds a genuine R-versus-C++ comparison for the `lambda` search against `stats::optimize()` over the `nlme::lme`-based likelihood — the tests previously named for this compared the C++ kernel against itself, since `optimal_parameter()` delegates to `optimal_parameter_cpp()`.
* `fh()` gained a `cpus` argument positioned before `seed`, so code calling `fh()` with 20 positional arguments should switch to named arguments.
