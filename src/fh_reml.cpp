...
// optimize(A.reml, interval, maximum = TRUE). The minimiser is R's Brent_fmin,
// transcribed in brent.h.
// [[Rcpp::export]]
double fh_estsigmau2_reml_cpp(const arma::vec& direct,
  ...
  return emdi::brent_fmin(
    lower, upper,
    [&](double s2) { return -fh_reml_loglik_cpp(s2, direct, X, vardir); },
    tol
  );
}
